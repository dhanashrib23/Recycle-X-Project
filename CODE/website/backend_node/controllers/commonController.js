...
const getAllTrashSubCategoriesbycatid = (request, response) => {
  const { categoryId } = request.params; // Get categoryId from request parameters
  const statement = `SELECT * FROM ${supplier.TRASH_SUBCATEGORIES} WHERE category_id = ?`;

  db.execute(statement, [categoryId], (error, result) => {
    if (error) {
      response
        .status(500)
        .json(
        ...
        response
          ...
          .json(
            reply.onSuccess(
              200,
              result,
              "No subcategories found for the provided category."
            )
          );
      } else {
        response
